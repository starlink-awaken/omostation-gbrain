...
// F7114ABA SRP (2026-06-27): engine 专属类型提取到 engine-types.ts (God Module 拆分)
// BrainEngine interface 用的 engine 专属类型 (引入本模块作用域)
import type {
  TraverseGraphOpts, FileRow, FileSpec, LinkBatchInput, TimelineBatchInput,
  ReservedConnection, TakeBatchInput, Take, TakesListOpts, TakeHit, StaleTakeRow,
  TakeResolution, TakesScorecard, TakesScorecardOpts, CalibrationBucket,
  CalibrationCurveOpts, SynthesisEvidenceInput, DreamVerdict, DreamVerdictInput,
  FactInsertStatus, FactRow, NewFact, FactListOpts, FactsHealth,
  TrajectoryOpts, TrajectoryPoint,
} from './engine-types.ts';
// 向后兼容: 消费者 import { FileRow, BrainEngine, ... } from "./engine" 仍工作
