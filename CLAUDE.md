...
- `src/core/memu-engine.ts` (memu B++ 2026-06-28) — Bun SQLite 轻量嵌入式 engine (第 4 种, 平级 postgres/pglite). 原 b245c11a 半成品 (99 typecheck 错) 完整修复: import 拆分 (engine.ts/engine-types.ts/types.ts) + 接口边界字段 snake_case (body→compiled_truth 等) + 53 方法签名 rest args (`Parameters<BrainEngine['X']>` 杀手锏) + 返回类型对齐 (batch→number/contradictions→boolean). BrainEngine 54 方法实现 (核心 CRUD 可用, searchVector/traverseGraph 等 stub return 空). `test/memu-smoke.test.ts` 2/2 (createEngine+initSchema+putPage+getPage+addLink+config+stats). 激活: `engine: 'memu'` + `database_path`. engine-factory `case 'memu'` 分支. commit 538c0c42.
