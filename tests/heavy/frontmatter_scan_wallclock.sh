...
# --no-embedding: this is a doctor frontmatter-scan wallclock smoke, no
# embeddings are used, and CI has no embedding provider key configured.
# Without it gbrain init fails with "No embedding provider configured".
